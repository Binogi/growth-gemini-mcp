...
export function registerDeepResearchTool(server) {
    ...
    server.tool('gemini-deep-research', 'Start an autonomous Gemini Deep Research task (5-60 minutes, runs on Google servers). Returns a research ID. ' +
        'Nothing is written to disk by this tool and no background polling runs in this server: the result JSON only ' +
        'appears once gemini-check-research observes completion, under a timestamped name, so poll with ' +
        'gemini-check-research rather than watching GEMINI_OUTPUT_DIR.', {
...
**Note:** The research runs on Google's servers; this server does not poll in the background. The result JSON is written to the output directory only when \`gemini-check-research\` observes completion, under a timestamped name, so poll rather than watch the directory.`,
                    },
                ],
            };
        }
...
- Wait for Interactions API to become available in your region`,
    ...
    server.tool('gemini-check-research', 'Check a Deep Research task. On the first call that observes completion, writes the full JSON to ' +
...
Check again in 30-60 seconds using \`gemini-check-research\`.`,
    ...
    server.tool('gemini-research-followup', 'Ask a follow-up question about a completed Deep Research task; the answer is generated by the configured pro ' +
        'model (GEMINI_PRO_MODEL) with the research as context. Returns text only; nothing is written to disk.', {
